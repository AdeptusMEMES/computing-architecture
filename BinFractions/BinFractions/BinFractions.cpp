// BinFractions.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
int* decToBinFrac(float num)
{
	...
	if (E > 253)
		overflowMes = 1;
	float Mreal = (num - powf(2, (float)i)) / (powf(2, (float)(i + 1)) - powf(2, (float)i))*powf(2, 23);
...
int main()
{
	...
	if (shiftMes == 1)
	{
		cout << "\n ERROR! FRACTION PART IS LOST BECAUSE INTEGER PART IS TOO BIG! \n\n";
		return 1;
	}

	cout << "First real binary number: ";
	prntBin(decToBinFrac(x), 32);
	cout << "Second real binary number: ";
	prntBin(decToBinFrac(y), 32);
	cout << "Binary sum: ";
	prntBin(sum, 32);
	cout << "Decimal sum: ";
	cout << scientific << binToDec(sum);
	cout << "\nBinary substruction: ";
	prntBin(sub, 32);
	cout <<  "Decimal substruction: ";
	cout << scientific << binToDec(sub);
}